...
#x=read.delim(sprintf("%s_clusters.tsv", organism.dir))
#for (i in c('resid','dens_string','meanp_meme')) x[[i]] = as.numeric(gsub('f0','',as.character(x[[i]])))
if ( ! exists('ratios') ) attach( global.data )
if ( ! exists('n.motifs') ) n.motifs <- max(params$iter.n.motifs, na.rm=T)
rm( mkBgFile, runMast, mkTempMemeFiles, get.mast.pvals, getMastPValuesAndEValues ) ## old version conflicts with new version
...
e$cmonkey.re.seed( e )
try(sys.source("~/scratch/biclust/cmonkey-funcs.R",envir=e,chdir=T))
...
##rm(all.seqs,clust,bg.list,tmp2,bg.fname); gc()
...
for ( i in 1:length(tmp) ) {
  ...
  e$meme.scores[[seq.type]][[i]] = list( k=i, last.run=FALSE, meme.out=clust$meme.out, pv.ev=tmp[[i]]$pv.ev, 
	prev.run=NULL )
...
save( e, file=sprintf('output/%s_out.RData', organism) )
